...
model User {
  ...
  jobsDriven          Job[]             @relation("jobsDriven")
  jobEvents           JobEvent[]
}
...
model Job {
  ...
  // Who created the job (dispatcher/requester/admin)
  createdBy   User   @relation("jobsCreated", fields: [createdById], references: [id])
  createdById String

  // Assigned driver (nullable until assigned)
  driver User? @relation("jobsDriven", fields: [driverId], references: [id])

  // Basic info
  title       String
  description String?
  company     String?
  priority    String  @default("MEDIUM") // LOW/MEDIUM/HIGH as string
  status      String  @default("NEW") // NEW/ASSIGNED/EN_ROUTE_PICKUP/...
...
model JobEvent {
  ...
  eventType   String // CREATED, ASSIGNED, STATUS_CHANGED, UPDATED, NOTE_ADDED, etc
  payloadJson String @default("{}")
